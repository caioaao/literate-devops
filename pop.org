...
** Android studio
   ...
   #+begin_src shell :dir /tmp :var version=android-studio-version build=android-studio-build
   curl -sL https://redirector.gvt1.com/edgedl/android/studio/ide-zips/${version}/android-studio-ide-${build}-linux.tar.gz | tar -xvz
   #+end_src

   #+begin_src shell :dir /sudo:: :var build=android-studio-build
   mv /tmp/android-studio /opt/android-studio-${build}
   rm -f /opt/android-studio
   ln -sf /opt/android-studio-${build} /opt/android-studio
   #+end_src
...
** bosh-cli
   ...
   #+begin_src shell :dir /sudo::
   apt-get install -y build-essential zlibc zlib1g-dev ruby ruby-dev openssl \
       libxslt1-dev libxml2-dev libssl-dev libreadline7 libreadline-dev      \
       libyaml-dev libsqlite3-dev sqlite3
   #+end_src

   Download the CLI:

   #+begin_src shell :dir /tmp :var version=bosh-cli-version
   wget https://github.com/cloudfoundry/bosh-cli/releases/download/v${version}/bosh-cli-${version}-linux-amd64
   #+end_src

   And move it:

   #+begin_src shell :dir /sudo:: :var version=bosh-cli-version
   mv /tmp/bosh-cli-${version}-linux-amd64 /usr/local/bin/bosh-cli-${version}
   chmod +x /usr/local/bin/bosh-cli-${version}
   ln -sf /usr/local/bin/bosh-cli-${version} /usr/local/bin/bosh-cli
   #+end_src
...
** control-tower
   ...
   #+begin_src shell :dir /tmp :var version=control-tower-version
   wget https://github.com/EngineerBetter/control-tower/releases/download/${version}/control-tower-linux-amd64
   #+end_src

   And move it:

   #+begin_src shell :dir /sudo:: :var version=control-tower-version
   mv /tmp/control-tower-linux-amd64 /usr/local/bin/control-tower-${version}
   chmod +x /usr/local/bin/control-tower-${version}
   ln -sf /usr/local/bin/control-tower-${version} /usr/local/bin/control-tower
   #+end_src
...
** PlantUML
   ...
   Let's create an alias for that:

   #+begin_src shell :tangle ~/.zsh.d/apps.d/10-plantuml.sh
   #!/usr/bin/env bash
   alias plantuml="java -jar ${HOME}/utils/jars/plantuml.jar"
   #+end_src
