...
** iptables firewall
   ...
   #+begin_src shell :dir /sudo::
     ...
     # Rules to allow outgoing traffic to docker containers
     iptables --append FORWARD -i docker+ -o enp+ -j ACCEPT
     iptables --append FORWARD -i docker+ -o wlp+ -j ACCEPT
     iptables --append FORWARD -i enp+ -o docker+ --match state --state ESTABLISHED,RELATED -j ACCEPT
     iptables --append FORWARD -i wlp+ -o docker+ --match state --state ESTABLISHED,RELATED -j ACCEPT
