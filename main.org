...
Now some cool scripts:

  - Notify when an execution ends.

    #+BEGIN_SRC shell :tangle ~/.local/bin/exec_notify :tangle-mode (identity #o755)
    #!/usr/bin/env bash
...
#+begin_src shell :tangle ~/.zsh.d/apps.d/00-gpg-agent.sh
export GPG_TTY=$(tty)
...
yubikey. [[https://ludovicrousseau.blogspot.com/2019/06/gnupg-and-pcsc-conflicts.html][This blog post]] goes more into detail.
