...
Another thing, let's just enable fingerprint auth. For some reason it comes disabled in authselect's default profile.
...
#+begin_src shell :dir /tmp
curl -fsSL https://downloads.nordcdn.com/apps/macos/generic/NordVPN-OpenVPN/latest/NordVPN.pkg -o nordvpn.pkg
open nordvpn.pkg
#+end_src
*** Configure
...
Since the repo is still private, it's easier to just go to [[https://github.com/mitchellh/ghostty/releases/download/tip/ghostty-macos-universal.zip][this link]], download and install it manually.

After that, run:

#+begin_src shell :dir /sudo:: :var user=(user-login-name)
mv /Applications/Ghostty.app /tmp/Ghostty.app.bkp
mv ~${user}/Downloads/Ghostty.app /Applications/Ghostty.app
#+end_src
...
#+begin_src shell :dir /sudo::
dnf install libadwaita-devel gtk4-devel
#+end_src

And download the nightly version of Zig:

#+begin_src shell :dir /tmp
mkdir zig
curl -fsSL https://ziglang.org/builds/zig-linux-x86_64-0.12.0-dev.1808+69195d0cd.tar.xz | tar -xJC zig --strip-components=1
#+end_src
...
git checkout tip
direnv allow
/tmp/zig/zig build -p ~/.local
#+end_src

**** Configure

#+begin_src config :tangle ~/.config/ghostty/config :mkdirp yes
...
curl -fsSL https://dl.google.com/dl/cloudsdk/channels/rapid/downloads/google-cloud-cli-454.0.0-darwin-arm.tar.gz | tar -xz
#+end_src
...
#+name: elixir_ls_version
: 0.19.0

#+begin_src shell :dir ~/.local/elixir-ls :mkdirp yes :var version=elixir_ls_version
rm -rf *
curl -fsSL https://github.com/elixir-lsp/elixir-ls/releases/download/v${version}/elixir-ls-v${version}.zip > /tmp/elixir-ls.zip
...
#+begin_src shell :tangle ~/.config/zsh.d/apps.d/10-awscli.sh
